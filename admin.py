# -*- coding: utf-8 -*-
"""QGIS Animation Workbench plugin admin operations"""
...
@app.command()
def symlink(context: typer.Context, from_source: bool = False, build_first: bool = True):
    """Create a plugin symlink to QGIS plugins directory

    :param context: Application context
    :type context: typer.Context

    :param from_source: If True, symlink directly from source directory
        (useful for live development). If False, symlink from build directory.
    :type from_source: bool

    :param build_first: If True and not using --from-source, build the plugin
        before creating the symlink. Default is True.
    :type build_first: bool
    """
    # Uninstall any existing plugin first
    _log("Removing any existing plugin installation...", context=context)
    uninstall(context)

    # Determine source path
    if from_source:
        source_path = LOCAL_ROOT_DIR / SRC_NAME
        _log(f"Symlinking from source directory: {source_path}", context=context)
    else:
        if build_first:
            _log("Building plugin first...", context=context)
            build(context, clean=True)
        source_path = LOCAL_ROOT_DIR / "build" / SRC_NAME
        _log(f"Symlinking from build directory: {source_path}", context=context)

    if not source_path.exists():
        typer.echo(f"Error: Source path does not exist: {source_path}", err=True)
        raise typer.Exit(code=1)

    # Determine QGIS plugins directory (platform-aware)
    if os.name == "nt":
        plugin_path = os.path.join(
            "AppData",
            "Roaming",
            ...
            "QGIS3",
            "profiles",
            context.obj["qgis_profile"],
        )
        root_directory = Path(os.environ["USERPROFILE"]) / plugin_path
    else:
        root_directory = Path.home() / f".local/share/QGIS/QGIS3/profiles/{context.obj['qgis_profile']}"
    ...
    # Create symlink
    if destination_path.exists() or os.path.islink(destination_path):
        _log(f"Removing existing path: {destination_path}", context=context)
        if os.path.islink(destination_path):
            os.unlink(destination_path)
        else:
            shutil.rmtree(str(destination_path), ignore_errors=True)

    os.symlink(source_path, destination_path)
    _log(f"Created symlink: {destination_path} -> {source_path}", context=context)
...
def _get_existing_releases(
    ...
    """
    base_url = "https://api.github.com/repos/" "timlinux/QGISAnimationWorkbench/releases"
