...
do
  TRANSLATION_FILE="animation_workbench/i18n/$LOCALE.ts"
...
then
  ...
  for LOCALE in ${LOCALES}
  do
    echo "animation_workbench/i18n/"${LOCALE}".ts"
    # Note we don't use pylupdate with qt .pro file approach as it is flakey
    # about what is made available.
    ${PYLUPDATE} -noobsolete ${PYTHON_FILES} -ts animation_workbench/i18n/${LOCALE}.ts
  done
else
